...
def wait_for_result(server: str, prompt_id: str, poll_interval=2, timeout=600):
    ...
    while elapsed < timeout:
        ...
        if prompt_id in history:
            ...
            # collect ALL saved images; prefer the final over _stage1 intermediates
            candidates = []
            for node_output in outputs.values():
                for img in node_output.get("images", []):
                    candidates.append(img)
            if candidates:
                finals = [i for i in candidates if "_stage1" not in i["filename"]]
                img = (finals or candidates)[0]
